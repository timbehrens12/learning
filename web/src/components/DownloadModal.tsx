...
export const DownloadModal = ({ isOpen, onClose }: DownloadModalProps) => {
  ...
  const handleGoogleSignUp = async () => {
    if (!supabase) {
      ...
      alert('Authentication is not available. Please download the app to sign up.');
      handleDownload();
      return;
    }
              ...
              <GlassCard className="p-8 border-white/10 relative">
                ...
                <div className="space-y-3">
                  ...
                  <button
                    ...
                    disabled={isLoading}
                    className="w-full py-3 rounded-xl font-semibold text-sm bg-white/5 hover:bg-white/10 border border-white/10 text-white flex items-center justify-center gap-2 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    {isLoading ? (
                      <>
